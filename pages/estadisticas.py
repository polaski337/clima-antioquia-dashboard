...
def main() -> None:
    ...
    with tab_temporal:
        ...
        with left:
            st.plotly_chart(line_chart(filtered, selected_variable), use_container_width=True)
        with right:
            st.plotly_chart(precipitation_chart(filtered), use_container_width=True)

    with tab_municipios:
        st.plotly_chart(municipality_bar_chart(filtered, selected_variable), use_container_width=True)

    with tab_heatmap:
        st.plotly_chart(heatmap_chart(filtered, selected_variable), use_container_width=True)
