...
def main() -> None:
    ...
    with left:
        ...
        st.plotly_chart(line_chart(filtered, variable), use_container_width=True, theme=None, config=plotly_config())
    with right:
        st.markdown('<div class="section-title">Precipitación acumulada</div>', unsafe_allow_html=True)
        st.plotly_chart(precipitation_chart(filtered), use_container_width=True, theme=None, config=plotly_config())
